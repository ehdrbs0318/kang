...
///
/// **Task 12 에서 도그푸딩 코퍼스로 빈도를 실측했다** — 저장소 자신의 `plans/`·`docs/`
/// 산문 2035 줄, 백틱 조각이 2개 이상인 줄 216, 원문이 `.` 으로 이은 계층 이름 17종,
/// 계층 상위 조각이 인접 쌍의 앞에 단독으로 온 자리(근접 위험) 7. **실제 충돌은 0 건이다.**
/// 그래서 승급하지 않았다. **올릴 조건은 실제 코퍼스에서 충돌 1 건이 관측되는 것이다.**
