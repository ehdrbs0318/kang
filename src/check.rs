...
//!
//! **그래프 값을 남기지 않는다.** 참조 전파는 v2 이므로 v1 에 질의할 소비자가 없고,
//! `show` 의 재귀 임베드는 `imports` 를 직접 순회한다 (스펙 5.3).
//!
//! # 보고 규약
//!
//! 순환 하나를 어디서 시작해 보고할지는 정해야 하는 문제다 — `A→B→C→A` 와
//! `B→C→A→B` 는 같은 순환이다. 세 가지로 못박는다.
//!
//! - **되돌아오는 간선 하나에 진단 하나.** DFS 가 아직 스택에 있는 문서를 다시 만나는
//!   순간이 순환이 닫히는 자리이고, 체인은 그 문서부터 지금까지의 스택이다. 같은 순환을
//!   두 번 보고하지 않으려면 세는 단위가 필요하고, 되돌아오는 간선이 그 단위다.
//!   겹치는 순환(`A↔B` 와 `B↔C`)은 닫는 간선이 둘이므로 진단도 둘이다.
//! - **시작점은 문서 경로 순.** 정렬하지 않으면 `HashMap` 의 나열 순서에 따라 같은
//!   프로젝트가 실행마다 다른 체인을 낸다.
//! - **같은 대상으로 가는 import 가 여럿이면 파일에서 먼저 나온 줄만 본다.** 간선은
//!   둘이지만 순환은 하나이고, 둘 다 따라가면 같은 순환이 두 번 보고된다.
//!
...
fn 순환(체인: &[(&DocPath, usize)]) -> Diagnostic {
    ...
    Diagnostic {
        ...
        locations: 체인
            ...
            .collect(),
        fixes: vec![끊는_수정(체인, 시작_문서)],
    }
}

/// 순환을 끊는 수정을 만든다. 고리를 닫는 import 줄을 지우는 것이 가장 작은 편집이다.
///
/// 줄 번호를 좌표로 쓰지 않는다 (ADR-0003) — 무엇을 지울지는 import 대상으로 지정한다.
///
/// # 매개변수
/// - `체인`: 순환에 든 문서들. 마지막 문서의 import 가 고리를 닫는다
/// - `시작_문서`: 고리가 닫히며 다시 가리켜지는 문서
///
/// # 반환값
/// 고리를 닫는 문서를 고치는 `Edit` 수정
fn 끊는_수정(체인: &[(&DocPath, usize)], 시작_문서: &DocPath) -> Fix {
    let 닫는_문서 = 체인
        .last()
        .expect("순환 체인에는 고리를 닫는 문서가 최소 하나 있다")
        .0;

    // 체인이 하나면 자기 자신을 import 한 것이다. 문서가 하나뿐이라 "공통 개념을 상위
    // 문서로" 는 뜻이 없고, 자기 심볼은 import 없이 쓸 수 있으므로 줄을 지우면 끝난다.
    let action = if 체인.len() == 1 {
        "이 문서는 자기 자신을 import 하고 있습니다. 자기 문서가 선언한 심볼은 import 없이 그대로 쓸 수 있으므로 이 import 줄을 지우세요.".to_string()
    } else {
        format!(
            "이 문서의 import 줄 가운데 대상이 {시작_문서} 인 것을 지워 순환을 끊으세요. 순환에 든 문서들이 함께 쓰는 개념이 있다면, 그 개념만 담은 상위 문서를 새로 만들어 각 문서가 그것을 import 하게 하세요."
        )
    };
