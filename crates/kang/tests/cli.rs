...
fn 임시_파일_자리의_심볼릭_링크를_따라가지_않는다() {
    ...
    assert_eq!(
        코드, 2,
        "링크가 막고 있으면 환경 오류로 끝나야 한다: {stderr}"
    );
    assert_eq!(stdout, "", "실패했으면 stdout 은 비어야 한다");
...
fn 임시_파일이_이미_있으면_열지_않고_그_사정을_말한다() {
    ...
    let 임시_자리 = root.join(".kang/index.tsv.tmp");
    fs::write(&임시_자리, "겹쳐 도는 다른 실행이 쓰던 것")
        .expect("임시 파일을 만들 수 있어야 한다");
...
/// 파싱에 실패하는 초안만 있는 프로젝트에 예제 템플릿을 만들면 안 된다.
///
/// `resolve::load` 는 **파싱에 성공한** 문서만 돌려준다. 그것만 보면 깨진 초안뿐인
/// 프로젝트가 "문서가 없다" 로 읽히고, 사용자가 쓰다 만 문서 옆에 남의 예제가 생긴다.
/// 그 사람에게 필요한 것은 예제가 아니라 자기 문서의 진단이다.
#[test]
fn init_은_깨진_초안이_있으면_예제를_만들지_않는다() {
    let root = 임시_루트("init_깨진_초안");
    git_저장소로(&root);
    // frontmatter 가 없어 K102 로 파싱에 실패한다. `docs/example.kang` 이 아닌 경로다 —
    // 그 경로 존재 검사만으로는 걸리지 않는 자리를 고른다.
    쓰기(&root, "docs/초안.kang", "쓰다 만 초안이다\n");

    let (_, stderr, 코드) = 실행(&root, &["init"]);
    assert_eq!(코드, 0, "init 은 성공해야 한다: {stderr}");
    ...
    assert!(
        stderr.contains("이미 .kang 문서가 있어 건너뜁니다"),
        "건너뛴 사실을 말해야 한다: {stderr}"
    );
    // 초안은 손대지 않는다.
    assert_eq!(
        fs::read_to_string(root.join("docs/초안.kang")).expect("초안이 남아야 한다"),
        "쓰다 만 초안이다\n",
        "사용자 문서를 건드렸다"
    );
...
/// 한 줄의 백틱 조각이 많아도 빌드가 서지 않아야 한다.
///
/// `이름_분할` 의 DP 는 m 개 자리마다 최대 m 개 길이를 보고, 각 탐침이 조각들을 이어
/// 붙여 해싱했다 — 사실상 O(m³) 이다. 컴파일러는 저장소가 주는 아무 내용에나 이것을
/// 돌리므로, `.kang` 한 줄로 `build`·`show`·`index` 를 갈아 세울 수 있었다.
///
/// 실측(수정 전 → 후): 조각 1200 에서 419ms → 20ms, 4800 에서 (추정 수십 초) → 39ms.
///
/// 시간 상한은 넉넉하게 둔다 — 이 시험이 재는 것은 "빠른가" 가 아니라 **"차수가
/// 터지지 않는가"** 이고, 느린 CI 러너에서 깜빡이면 안 된다. 수정 전 코드는 이 조각
/// 수에서 수십 초가 걸려 상한에 걸린다.
#[test]
fn 한_줄의_조각이_많아도_빌드가_서지_않는다() {
    let root = 임시_루트("병합_차수");
    git_저장소로(&root);

    // 계층 keyword 를 선언해야 병합 탐색이 실제로 돈다. 선언이 없으면 DP 가 곧바로
    // 빠져나가 이 시험이 아무것도 재지 못한다.
    let 조각 = 1200;
    let mut 문서 = String::from("---\ndescription: 병합 차수 시험\n---\n\nkeyword `a`: 상위\n");
    // 하위 keyword 를 선언한다. 본문의 `` `a` `bN` `` 이 `a.bN` 으로 합쳐질 후보다.
    for i in 0..조각 {
        문서.push_str(&format!("keyword `a`.`b{i}`: 하위\n"));
    }
    문서.push_str("\n## 첫 정책\n\n");
    // 전부 **한 줄**에 둔다. 합칠 수 있는 것은 같은 줄에서 이어지는 조각뿐이다.
    for i in 0..조각 {
        문서.push_str(&format!("`a` `b{i}` "));
    }
    문서.push('\n');
    쓰기(&root, "docs/a.kang", &문서);
    ...
    assert_eq!(코드, 0, "정상 프로젝트다: {stderr}");
    assert!(
        걸림 < std::time::Duration::from_secs(10),
        "조각 {조각} 개에 {걸림:?} 가 걸렸다 — 병합 탐색의 차수가 터졌다"
    );
