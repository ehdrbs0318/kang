...
fn main() {
    ...
    // 만드는 자리**다.
    //
    // kang 의 플래그는 셋이고 각각 자리가 하나뿐이다 — `--help` 와 `--version` 은 단독
    // 인자, `--import` 는 `bless` 의 셋째 인자. 그 밖의 `-` 로 시작하는 인자는 전부 사용법
    // 오류다. 문서·심볼 이름이 `-` 로 시작하면 CLI 로 가리킬 수 없고, 그때 처방은
    // 스펙 6.0 과 같다 — 주소를 댈 수 없는 이름은 이름을 고친다.
    let 제자리_플래그 = |자리: usize| match 조각[자리] {
        "--help" | "--version" => 조각.len() == 1,
    ...
    let 코드 = match 조각.as_slice() {
        ...
        // 릴리즈 바이너리를 curl 로 받은 소비자는 그것이 어느 계약인지 알 방법이 이것뿐이다
        // — 저장소도 Cargo.toml 도 손에 없다. `show` 의 YAML 스키마·종료 코드·진단 형식·
        // 인덱스 TSV 는 0.x 동안 바뀔 수 있다고 CHANGELOG 가 명시하므로, 쥔 것이 무엇인지
        // 말할 수 없는 바이너리는 그 계약을 지킬 수 없다.
        ["--version"] => {
            println!("kang {}", env!("CARGO_PKG_VERSION"));
            0
        }
        ["build"] => match compile() {
            Ok(_) => 0,
