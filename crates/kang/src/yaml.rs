...
impl Emitter {
    ...
    /// `키: true` / `키: false` 한 줄을 넣는다.
    ///
    /// **bool 은 [`scalar`] 를 거치지 않는다.** 거치게 하려면 `"true"` 를 인용에서 면제해야
    /// 하는데, [`Emitter::pair`] 가 **모든** 값을 [`scalar`] 로 보내므로 그 면제는 이름·경로·
    /// 정의까지 샌다. 그러면 `true` 라는 이름의 심볼이 bool 로 나간다.
    ///
    /// # 매개변수
    /// - `key`: 스키마가 정한 키
    /// - `value`: 그대로 나갈 bool
    ...
    pub fn block(&mut self, key: &str, body: &str) {
        ...
        // **literal scalar 에는 이스케이프 수단이 없다.** 파서가 거부하는 문자(제어 문자)나
        // 우리가 줄로 세지 않는데 파서는 줄로 읽는 문자(`\r`·U+2028·U+2029)가 본문에 있으면
        // 인용 스칼라로 물러나는 것 외에 방법이 없다. 물러나지 않으면 문서 **전체**가
        // 파싱되지 않는다 — 터미널 출력을 붙여 넣은 본문의 ANSI 이스케이프가 그 입구다.
        //
        // `\n` 과 `\t` 는 제외한다. 개행은 블록의 구조 그 자체이고, 탭이 든 본문(들여쓴
        // 코드)은 지금 그대로 왕복한다. 여기서 탭까지 물러나면 흔한 마크다운이 통째로
        // 한 줄짜리 인용 스칼라로 떨어진다.
        if body
            .chars()
            .any(|c| (c.is_control() && c != '\n' && c != '\t') || 줄_구분자(c))
        {
            self.lines.push(format!("{key}: {}", scalar(body)));
            return;
        }
    ...
    pub fn seq(&mut self, key: &str, items: impl IntoIterator<Item = Emitter>) {
        ...
        for item in items {
            // 첫 줄이 빈 항목은 `- ` 를 잃고 **앞 항목의 매핑에 조용히 병합된다** —
            // 항목 하나가 통째로 사라지는데 파서는 오류를 내지 않는다. 지금은 `pair`·
            // `block`·`seq`·`map`·`value` 중 어느 것도 빈 첫 줄을 만들지 않지만,
            // 이 모듈이 "짝 어긋남이 구조적으로 불가능" 하다고 말하는 유일한 예외 자리다.
            debug_assert!(
                item.lines.first().is_none_or(|첫| !첫.is_empty()),
                "목록 항목의 첫 줄은 비어 있을 수 없다"
            );
...
///
/// - 빈 문자열, 앞뒤 공백, 제어 문자 (개행 포함), 줄 구분자 ([`줄_구분자`])
/// - 지시자로 시작 (`- ? : , [ ] { } # & * ! | > ' " % @ \` \` 와 역슬래시)
/// - `: ` 를 포함하거나 `:` 로 끝남 — **한글 description 의 위험 지점이다.**
///   `결제: 대금을 지불하는 행위` 를 그대로 두면 그 자리에서 매핑이 하나 더 생긴다
/// - ` #` 를 포함 — 뒤가 주석이 된다
/// - 정확히 `=` — YAML 1.1 의 value 키라 `safe_load` 가 문서 전체를 거부한다
/// - 숫자로 읽힐 표기(숫자·`+`·`.` 로 시작)와 null/bool 표기
///
/// **면제가 없다.** `true` 도 인용한다 — 스펙 6.4 의 bool 자리는 [`Emitter::flag`] 가
/// 따로 내며, 여기에 면제를 두면 [`Emitter::pair`] 를 지나는 **모든** 값이 그 면제를
/// 함께 받아 `true` 라는 이름의 심볼이 bool 로 나간다.
///
/// ponytail: YAML 스펙의 전체 규칙이 아니라 **이 스키마에 필요한 만큼**이다. 값에 오는
/// 것은 문서 경로·심볼 이름·한 줄 정의뿐이며, 그 셋에서 구조를 깨거나 타입을 바꾸는
/// 표기만 본다. 판단이 갈리면 인용하는 쪽으로 기운다 — 불필요한 인용은 읽기만 나빠지지만
/// 빠뜨린 인용은 문서를 깬다. **이 목록이 완전하다는 보장은 없다** — `=` 와 U+2028 이
/// 리뷰에서 나왔듯 규칙은 하나씩 는다. `tests/yaml.rs` 의 적대적 입력 목록이 그 기록이며,
/// 스키마에 자유 텍스트 필드가 늘어나면 이 규칙 대신 직렬화 크레이트를 쓴다.
///
/// # 매개변수
/// - `s`: 인용할 값
///
...
pub fn scalar(s: &str) -> String {
    ...
    let 인용 = s.is_empty()
        ...
        || s.ends_with(':')
        || s.contains(" #")
        // `=` 는 YAML 1.1 의 value 키다. PyYAML 의 SafeConstructor 에 그 태그의 생성자가
        // 없어 **문서 전체**가 거부된다. 정확히 이 한 글자일 때만이므로 `docs/h.=` 는 평문이다.
        || s == "="
        // `=` 는 YAML 1.1 의 value 키다. PyYAML 의 SafeConstructor 에 그 태그의 생성자가
        // 없어 **문서 전체**가 거부된다. 정확히 이 한 글자일 때만이므로 `docs/h.=` 는 평문이다.
        || s.chars().any(|c| c.is_control() || 줄_구분자(c))
        || 타입으로_읽힘;
    ...
    for c in s.chars() {
        match c {
            ...
            // U+2028·U+2029 는 두 자리 `\x` 로 적을 수 없다. 이스케이프하지 않으면
            // 큰따옴표 안에서도 줄바꿈으로 읽혀 인용이 그 자리에서 끊긴다.
            c if 줄_구분자(c) => 결과.push_str(&format!("\\u{:04x}", c as u32)),
            // U+2028·U+2029 는 두 자리 `\x` 로 적을 수 없다. 이스케이프하지 않으면
            // 큰따옴표 안에서도 줄바꿈으로 읽혀 인용이 그 자리에서 끊긴다.
            c => 결과.push(c),
        }
...
/// 이 문자를 YAML 파서가 줄바꿈으로 읽는가.
///
/// Rust 의 [`char::is_control`] 은 유니코드 `Cc` 만 판정하므로 U+2028(`Zl`)·U+2029(`Zp`)를
/// 놓친다. 우리는 `\n` 만 줄로 세는데 파서는 이 둘도 줄로 세므로, 걸러 내지 않으면
/// **들여쓰기가 붙지 않은 줄**이 생겨 블록이나 인용이 그 자리에서 끝난다.
///
/// U+0085(NEL)는 `Cc` 이므로 [`char::is_control`] 이 이미 잡는다.
///
/// # 매개변수
/// - `c`: 검사할 문자
///
