...
//!
//! **rev 핀을 고쳐 쓰는 유일한 층이며, 파일 **안**의 바이트를 치환하는 유일한 층이다.**
//! ([`crate::init`] 도 사용자 파일에 쓰지만 marker 가 없는 파일 **끝**에 섹션을 덧붙이는
//! 것뿐이다. 그 모양은 최악이 "덜 쓰인 섹션" 이고 재실행이 복구이므로 아래 두 요구를
//! 지지 않는다 — 그 근거는 그 모듈에 적어 두었다.)
//!
...
impl ImportAddress {
    ...
    ///
    /// **문서 파일 이름 자체에 `.`·`#`·`!` 이 있으면 그 문자에서 갈린다** (`docs/a.b#정책`
    /// 은 keyword `docs/a`.`b#정책` 으로 읽힌다). 이것은 천장이 아니다 — 스펙 6.0 이 그
    /// 이름을 **불법으로 정했고**, 거절하는 것은 문서를 로드하는 층의 `K113` 이다.
    /// 여기서 `project.docs` 를 뒤져 최장 일치로 되살리는 것은 "구분자만으로 파싱한다" 는
    /// 스펙 지시를 어기고 경로 오타의 참인 진단을 망친다.
    ///
    /// # 매개변수
    /// - `s`: 백틱 없는 심볼 주소
    ///
