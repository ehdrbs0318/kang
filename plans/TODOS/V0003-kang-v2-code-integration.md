...
- **위치 보고는 주소가 아니다.** 1절의 "코드의 개수와 위치" 는 `파일:줄` 로 낸다. [ADR-0003](../../docs/adr/0003-symbolic-addressing-not-line-numbers.kang)이 `location` 을 명시적 예외로 두었다 — 저장되거나 다음 명령의 인자가 되는 값만 심볼이어야 하고, "여기를 보라" 는 표시는 휘발성이어도 된다. 코드 위치는 후자다.
...
**`declare` 가 아니라 본문 있는 함수로 낸다.** `declare` 는 "이 이름이 어딘가에 있다" 는
선언일 뿐 값을 만들지 않는다. 소비자가 `generated.ts` 에서 `kangTopic` 을 import 하면
타입 체크는 통과하지만 런타임에 `undefined` 가 온다. 아무것도 하지 않는 데코레이터를
실제로 내보내 그 구멍을 막는다. 구현은 `examples/ts-consumer/.kang/generated.ts` 가
그대로 보여 준다.
