...
- **`FixKind::Shell` 의 `action` 은 경로·이름을 반드시 인용한다.** 스펙 4.2·6.0·6.1 이 세 곳에서 못박은 조항이며 "인용 여부를 스스로 판단하게 두면 틀린다" 가 근거다. `format!("... ls -l {}", p)` 처럼 맨 보간을 하면 `/Users/x/My Project` 에서 셸이 두 인자로 쪼개 **원인 진단 대신 새로운 잘못된 사실**을 준다. 작은따옴표로 감싸되 값 안의 `'` 를 `'\''` 로 치환한다. Task 4 가 이 규약의 최초 구현이므로 이후 태스크는 그 모양을 그대로 따른다.
