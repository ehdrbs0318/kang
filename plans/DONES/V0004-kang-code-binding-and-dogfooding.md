...
- [x] **`Diagnostic` 에 `detail: Option<String>` 을 둘지 결정** — 스펙 5.1.1 의 3단 배치를 구현이 message 한 줄로 뭉쳤다. 세 요소는 전부 있으므로 스펙 요구는 충족되고 배치만 다르다. **사람의 판정이 필요하다**
  - **두지 않는다. 코드 0줄이고 스펙을 고쳤다.** 5.1.1 의 규범 문장은 세 **요소**를 요구하고(":257 세 요소가 항상 있다") 셋 모두 있다 — 갈린 것은 예시의 배치다
  - **기계 파싱 관점이 결정했다.** 소비자 둘이 `message` 를 쪼개지 않는다 — `examples/ts-consumer/test.mjs` 는 `stdio:"inherit"` 로 kang 의 stderr 를 그대로 흘려보내고 종료 코드와 생성 타입만 본다(`돌린다`·`핀을_대조한다`), `crates/kang-macros` 는 `.kang/index.tsv` 를 읽고 자기 진단을 `syn::Error` 로 낸다. **kang 의 진단 문면을 파싱하는 코드가 저장소에 0 건이다.** 설명을 머리글과 문단으로 나누면 읽는 쪽이 둘을 이어 붙이는 규칙을 새로 배워야 하므로 `detail` 은 파싱을 **어렵게** 한다
  - 비용도 반대쪽이다 — 36 자리 + 렌더러 분기 + 모든 message 재배치인데 요구한 소비자가 없다
  - 스펙 개정: 요소-자리-필드 대응 표 신설, 예시 셋을 **같은 주소의 픽스처로 실제 출력을 받아** 갈아 끼웠다, `fix (같은 개념일 때, 순서대로):` → `fix:`(조건은 머리글이 말한다), "왜 문제인지 한 문장" → "머리글 한 줄"(실제 message 는 여러 문장이다. `ast.rs`·`check.rs`·`tests/cli.rs` 의 같은 표현도 함께 고쳤다)
- [x] 진단 문체 통일 — `parse.rs` 의 note 는 마침표 없는 명사구·스펙 인용 0건, `check.rs`·`resolve.rs` 는 완전문·스펙 인용 있음. **한 실행에 두 문체가 섞이면 기계가 규칙을 하나로 배울 수 없다**
  - **마침표 없는 쪽으로 통일했다.** 스펙 5.1.1 의 렌더 예시 넷이 전부 그 문체이고, 값으로 끝나는 note(심볼 이름·운영체제 오류)에 마침표를 붙이면 그 마침표가 값의 일부로 읽히며(`.` 은 4.3 의 계층 구분자), 마침표는 사실을 담지 않으므로 떼는 쪽이 진단의 참을 건드리지 않는다
  - **note 18 개 + `[edit]` action 10 개**를 고쳤다. `message` 는 규칙 밖으로 두었다 — `K051` 의 message 는 자리 채움이 있어 `[shell]` 이 될 수 없는 `iconv` 명령으로 끝나고 그 뒤의 마침표는 경로를 깬다
  - **규칙을 `check::report` 하나에서 잰다** (`debug_assert!` 둘). 진단 전량이 그 함수를 지나므로 36종을 각각 재는 것과 같고 새 진단도 걸린다. 단언을 넣자 25 개 테스트가 실패하며 위반을 열거했고, 그중 `K040` 의 여러 줄 `format!` 은 소스 정규식 훑기가 놓친 것이다. **ponytail 천장:** 단언은 테스트가 실제로 찍은 15/33 종만 잰다 — 나머지는 소스의 note 39 자리·action 44 자리를 전부 훑어 닫았고, 승급 조건을 `report` 의 rustdoc 에 적었다
  - **컨트롤러 전제 정정:** "note 의 스펙 인용" 은 파일에 따라 갈리지 않는다. **어느 note 도 스펙을 인용하지 않으며**(3 파일 0 건) 인용은 message 에 있다 — `parse.rs` 2/11, `check.rs` 7/13, `resolve.rs` 1/7. 파일이 아니라 "근거를 댈 규칙이 있는가" 가 가른다
- [x] **stderr EPIPE** — `kang build 2>&1 | head` 가 exit 101. `찍기` 는 stdout 만 지키고 진단이 흐르는 stderr 는 무보호다(`eprint!` 34곳). `main.rs` 의 두 자리가 진단 전량을 내므로 그 둘만 태운다. **`set -euo pipefail` 이 CI 표준이라 실질 피해가 있다**
  - `진단_찍기` 하나로 두 자리를 태웠다. BrokenPipe 면 **그 시점 코드를 유지**하고 그 밖의 쓰기 실패만 안내 + 1 이다(안내도 같은 스트림이라 실패할 수 있어 결과를 버린다)
  - **실측 전/후:** 진단 400건(stderr 200KB) 프로젝트 `PIPESTATUS` 101 → 1. warn 만 있는 3000문서 프로젝트(stderr 2.2MB) **101 → 0**. 파이프 없이 돌린 종료 코드는 1·0 으로 불변
  - **`eprint!` 는 34곳이 아니라 37곳이다** (Task 3·7 이 `산출` 에 셋을 더했다). "두 자리가 진단 전량" 은 참이다 — `check::report` 호출이 `main.rs:181`·`main.rs:244` 둘뿐이다
  - 테스트는 세 종료 코드(0·1·2)를 함께 본다. 진단을 내는 자리가 둘이라 한 코드만 못박으면 `parse_project` 쪽을 놓친다. RED 는 `Some(101)` 이었다
- [x] `K051`(권한 없음·비 UTF-8)의 종료 코드를 1 → 2 로. 진단 자신의 fix 가 `ls -l`·`file -I` 인데 "문서를 고쳐야 한다" 로 분기된다
  - **2 로 옮겼다.** 기준은 원인이 문서 밖에 있는가다. 다른 error 와 섞여도 2 다 — 읽지 못한 파일이 있는 컴파일은 애초에 전부를 보지 못한 컴파일이므로 환경을 먼저 세워야 한다
  - 기존 테스트 4 자리(build 둘, bless 하나, fix 왕복 하나)의 기대를 2 로 바꾼 것이 RED 였다. 실측: 비 UTF-8 + 권한 없음 두 문서 프로젝트에서 `kang build` = 2
  - 스펙 6절 표의 괄호를 넓히고, `index`·`types` 의 "종료 코드 1" 문장에도 환경 오류 예외를 달았다(그 문장은 `K050` 에서 이미 부정확했다)
- [x] 스펙 5.1.1 에 **"적용하면 해소되는 fix" vs "확인만 하는 fix"** 를 성질로 구분하는 문장 — 지금 스펙에 그 구분이 없어 `ls -l` fix 의 옳고 그름을 판정할 근거가 없다
  - 「`fix` 의 두 성질」 절을 넣었다. 확인만 하는 fix 는 **두 경우에만** 쓴다 — 원인이 문서 밖일 때, 다음 명령의 인자를 컴파일러가 알 수 없을 때
  - **다중 후보 조건부 fix 는 앞의 성질이다.** 각 항목이 적용하면 해소되고 어느 갈래인지는 message 의 조건이 말한다 — 그래서 렌더된 머리글은 언제나 `fix:` 다
  - 종료 코드와의 관계를 한 방향으로만 적었다. 확인만 하는 fix 밖에 없으면 환경 오류이지만, **역은 성립하지 않는다** — `K050` 은 환경 오류이면서 `git init` 이라는 해소되는 fix 를 갖는다(테스트가 그 왕복을 잰다)

**수행 내역** — J1 은 코드 0줄(스펙 개정), J2 는 문자열 28 개 + 단언 둘, J3 은 헬퍼 하나,
J4 는 조건 한 줄. 테스트 340 → 341. `cargo test` 341 / clippy `--all-targets -D warnings` 0 /
`fmt --check` 0. 소비자 둘 다 통과 — `examples/ts-consumer` 의 `npm test` 4 단계 통과("핀 2개가
문서와 일치합니다"), `kang-macros` 12 + 2. 저장소 자신의 `kang build` 0.
리포트: `.superpowers/sdd/V0004-kang-code-binding-and-dogfooding/task-11-report.md`
